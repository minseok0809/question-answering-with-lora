...
def evaluate(export_dir: str = '.',
    ...
    if model is not None:
        ...
        def get_model_prediction_file(epoch_dir, prediction_file, split, _input_type, _output_type):
            path = pj(
                export_dir,
            ...
            output = lm.generate_prediction(epoch_dir, prediction_file, 
                raw_input, # (revised)
        ...
        try:
            hypothesis_file_test = get_model_prediction_file(epoch_dir, prediction_file, test_split, input_type, output_type)
        except ValueError:
            hypothesis_file_test = None
        try:
            hypothesis_file_dev = get_model_prediction_file(epoch_dir, prediction_file, validation_split, input_type, output_type)
        except ValueError:
