...
from .language_model_for_validation import TransformersQG
...
class Trainer:
    ...
    def __init__(self,
        ...
        self.config = Config(
            ...
            instruction_types=instruction_types, pretrained_model_path=pretrained_model_path, 
            ...
            for epoch in epochs:
                ...
                try:
                    ...
                    self.model = TransformersQG(
                        model=path, pretrained_model_path=self.pretrained_model_path, model_adapter=self.model_adapter, lora_rank=self.lora_rank,
        ...
        if not flag:
            ...
            self.model = TransformersQG(
                model=self.config.model, pretrained_model_path=self.pretrained_model_path, model_adapter=self.model_adapter, 
        ...
        self.data_cache_paths = [[(i, o, p, instruct), f'{prefix}.{i}.{o}.train.{instruct}.pkl']
                                 for i, o, p, instruct in zip(input_types, output_types, prefix_types, instruction_types)]


    def setup_optimizer(self, model, epoch: int = None):
        # optimizer
        optimizer = torch.optim.AdamW(model.model.parameters(), lr=self.config.lr)
        if epoch is not None:
            # load from existing config
            
            if 'model_' not in self.config.checkpoint_dir:
                path = pj(self.config.checkpoint_dir, "optimizers", f'optimizer.{epoch}.pt')
            elif 'model_' in self.config.checkpoint_dir:
                
                one_path = self.config.checkpoint_dir
                one_path = one_path.split("/")[0]
                print(one_path)
                path = pj(one_path, "optimizers", f'optimizer.{epoch}.pt')
            logging.info(f'load optimizer from {path}')
            device = 'cpu' if OPTIMIZER_ON_CPU else model.device
    ...
    def train(self, model_dir, epoch_save: None or int = 1, interval: int = 25, epoch_partial: int = None):
        ...
        with torch.cuda.amp.autocast(enabled=self.config.fp16):
            ...
            training_log_path = model_dir + '/valid_log.xlsx'
            validation_log_df = pd.DataFrame({'Epoch':[0], 'Loss':[0], 'Validation Time':[0]})

            for e in range(0, self.config.epoch):  # loop over the epoch 
                
                print("\n")
                mean_loss, global_step, elapsed_str = self.train_single_epoch(global_step, interval, e)
                logging.info(f"[epoch {e}/{self.config.epoch}] average loss: {round(mean_loss, 3)}")
                            # f"lr: {self.optimizer.param_groups[0]['lr']}")
                print("Loss :  ", round(mean_loss, 4))
                wandb.log({"loss": mean_loss})
                
                validation_log_df.loc[len(validation_log_df)] = [e + 1, round(mean_loss, 4), elapsed_str]
                validation_log_df = validation_log_df.astype({'Epoch':'int'})
    ...
    def train_single_epoch(self, global_step: int, interval, epoch):
        add_prefix = False if self.config.prefix_types is None else True
        model = TransformersQG(
            model=epoch + 1, pretrained_model_path=self.pretrained_model_path, model_adapter=self.model_adapter, lora_rank=self.lora_rank,
            lora_alpha=self.lora_alpha,
            max_length=self.config.max_length, max_length_output=self.config.max_length_output,
            label_smoothing=self.config.label_smoothing, add_prefix=add_prefix,
            drop_overflow_error_text=True, use_auth_token=self.use_auth_token, device_map=self.device_map,
            low_cpu_mem_usage=self.low_cpu_mem_usage, torch_dtype=self.torch_dtype)
        # self.optimizer = self.setup_optimizer(model, epoch)
        self.current_epoch = epoch
        assert self.current_epoch <= self.config.epoch, 'model training is done'
        ...
        encode_list = []
        for (i, o, p, instruct), cache_path in self.data_cache_paths:
            text_input, text_output, instruct_output = get_dataset(
                self.config.dataset_path, self.config.dataset_name, split='validation', input_type=i, output_type=o,
                instruction_type=instruct, use_auth_token=self.use_auth_token)
            encode_list += model.text_to_encode(text_input, text_output, prefix_type=instruct_output, cache_path=cache_path)
        data_loader = model.get_data_loader(encode_list, batch_size=self.config.batch, shuffle=True, drop_last=True)
        ...
        with tqdm(data_loader) as pbar:
            ...
            for n, encode in enumerate(pbar): 
                ...
                loss = model.encode_to_loss(encode)
