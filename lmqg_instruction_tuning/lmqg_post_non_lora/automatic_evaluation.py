...
def evaluate(export_dir: str = '.',
    ...
    reference_files = get_reference_files(dataset_path, dataset_name, instruction_type)
    ...
    if model is not None:
        ...
        def get_model_prediction_file(split, _input_type, _output_type, _instruction_type):
            path = pj(
                export_dir,
                f'samples.{split}.hyp.{_input_type}.{_output_type}.{dataset_path.replace("/", "_")}.{dataset_name}.txt')
            raw_input, _, raw_instruction = get_dataset(
                dataset_path,
                dataset_name,
                split=split,
            ...
            output = lm.generate_prediction(
                ...
                instruction_type=raw_instruction,
        ...
        try:
            hypothesis_file_test = get_model_prediction_file(test_split, input_type, output_type, instruction_type)
        except ValueError:
            hypothesis_file_test = None
        try:
            hypothesis_file_dev = get_model_prediction_file(validation_split, input_type, output_type, instruction_type)
        except ValueError:
