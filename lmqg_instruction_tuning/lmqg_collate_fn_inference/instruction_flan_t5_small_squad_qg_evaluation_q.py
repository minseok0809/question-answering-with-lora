...
def main():
    ...
    args = easydict.EasyDict({"checkpoint_dir": "tmp_instruction_flan_t5_small_squad_qg_q", 
                              
                              "project": "lmqg_qg_squad",

                              "prediction_file": "random_masking_small",
                              
                              "entity": "minseok0809",
                              
                              "name": "instruction_flan_t5_small_squad_qg_q",
                              ...
                              "max_length_output": 32, # (padding length)
    ...
    class GridSearcher:
        ...
        def __init__(self, checkpoint_dir: str = args.checkpoint_dir,
                     project: str = args.project, prediction_file: str = args.prediction_file,
                     entity: str = args.entity, name: str = args.name,
                    ...
                    question_types: List or str = args.question_types,
                    task_types: List or str = args.task_types,
            ...
            # static configs
            self.static_config = {
                'dataset_path': dataset_path, 'dataset_name': dataset_name, 'prediction_file': prediction_file, 
                ...
                'question_types':question_types, 'task_types': task_types, 'instruction_types': instruction_types
            }
        ...
        def get_evaluator(self, overwrite: bool):
            ...
            question_types = to_list(self.static_config['question_types'], sorting=False)   
            task_types = to_list(self.static_config['task_types'], sorting=False)   
            ...
            tmp = [(i, o, q, p, t, instruct) for i, o, q, p, t, instruct in zip(input_types, output_types, question_types, prefix_types, task_types, instruction_types)]
            if len(tmp) > 1:
                tmp = [(i, o, q, p, t, instruct) for i, o, q, p, t, instruct in tmp if o in ['question', 'questions_answers']]
            assert len(tmp) == 1, tmp
            i, o, q, p, t, instruct = tmp[0]
            prefix = pj(
                DEFAULT_CACHE_DIR,
                "encoded_feature",
            ...
            def get_evaluation(_checkpoint_dir_model):
                return evaluate(
                    ...
                    prediction_file=self.static_config['prediction_file'],
