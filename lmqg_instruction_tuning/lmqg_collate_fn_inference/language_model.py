...
def load_language_model(model_name,
    ...
    # https://blog.devgenius.io/fine-tuning-large-language-models-on-azure-machine-learning-358338f4e66a

    if (os.path.isfile(model_name + '/pytorch_model.bin') == True and os.path.isfile(model_name + '/adapter_model.bin') == False) or \
        (os.path.isfile(model_name + '/pytorch_model.bin') == False and os.path.isfile(model_name + '/adapter_model.bin') == False):
    ...
    elif os.path.isfile(model_name + '/adapter_model.bin') == True and os.path.isfile(model_name + '/pytorch_model.bin') == False:
        config = PeftConfig.from_pretrained(model_name, local_files_only=local_files_only, cache_dir=cache_dir, use_auth_token=use_auth_token) 
    ...
    try:
        ...
        if config.peft_type == 'LORA':
            if 't5' in model_name: 
                model_class = transformers.T5ForConditionalGeneration.from_pretrained 
                model = model_class(config.base_model_name_or_path)
...
class EncodePlus:
    ...
    def __init__(self,
        ...
        self.prefix = prefix_type if prefix_type is not None else None
    ...
    def encode_plus(self, input_sequence: str, output_sequence: str = None, input_highlight: str = None, \
                    question_sequence: str = None, answer_sequence: str = None, \
                        task_sequence: str = None, prefix_sequence: str = None):
        """ encode_plus
        ...
        if self.prefix is not None:
            ...
            if type(self.prefix) != str:    
                one_prefix = self.prefix[self.prefix_index]
                input_sequence = f'{one_prefix}: {input_sequence}'
        """
        prefix_sequence = self.prefix[self.prefix_index]
        ...
        if output_sequence is not None:
            ...
            encode['question'] = self.tokenizer.encode(question_sequence, **self.param_out)
            encode['answer'] = self.tokenizer.encode(answer_sequence, **self.param_out)
            encode['task'] = self.tokenizer.encode(task_sequence, **self.param_out)
            encode['prefix'] = self.tokenizer.encode(prefix_sequence, **self.param_out)
        return encode
...
class TransformersQG:
    ...
    def __init__(self,
        ...
        self.tokenizer, self.model, config = load_language_model(
            self.model_name, self.model_adapter, self.lora_rank, self.lora_alpha,
            cache_dir=cache_dir, use_auth_token=use_auth_token, device_map=device_map,
        ...
        if torch.cuda.device_count() > 1:
            self.parallel = True
            self.model = torch.nn.DataParallel(self.model, device_ids=[0]) # (revised)
            if self.model_ae is not None:
                self.model_ae = torch.nn.DataParallel(self.model_ae, device_ids=[0]) # (revised)
    ...
    def generate_prediction(self, epoch_dir, prediction_file, batch_size, split, 
       ...
       """
        ...
        input_types = to_list(input_types, sorting=False)
        output_types = to_list(output_types, sorting=False)
        question_types = to_list(question_types, sorting=False)
        task_types = to_list(task_types, sorting=False)
        instruction_types = to_list(instruction_types, sorting=False)
        assert len(input_types) == len(output_types)

        prefix_types = to_list("qg", sorting=False)
        prefix = pj(
            DEFAULT_CACHE_DIR,
            "encoded_feature"
            f"{'data_q'}",
            f"{'TransfomersQG'}.{'512'}.{'64'}"
        )
        ...
        for (i, o, p, q, t, instruct), cache_path in self.data_cache_paths:
            text_input, text_output, question_output, task_output, instruct_output = get_dataset(
                ...
                instruction_type=instruct, use_auth_token=False)
            
            encode_list += self.text_to_encode(text_input, text_output, \
                                                     question_type=question_output, prefix_type=instruct_output, \
                                                     task_type=task_output,
                                                     cache_path=cache_path)
  
        prediction_df_path = 'prediction/' + prediction_file + "_" + epoch_dir + "_" +  'prediction_test.xlsx'
    
        if os.path.isfile(prediction_df_path) == True:
            prediction_df_path = 'prediction/' + prediction_file + "_" + epoch_dir + "_" +  'prediction_validation.xlsx'
            
        elif os.path.isfile(prediction_df_path) == False: 
            prediction_df_path = 'prediction/' + prediction_file + "_" + epoch_dir + "_" +  'prediction_test.xlsx'
            
        prediction_df = pd.DataFrame({"Preidction": ["string"]}) 

        num = -1
        loader = self.get_data_loader(self.model, tokenizer, encode_list, batch_size = batch_size, shuffle = False, drop_last= False)

        outputs = []
        for idx, encode in enumerate(loader):
            # if idx <= 10:
            with torch.no_grad():
                if 'labels' in encode:
                    encode.pop('labels')
                encode = {k: v.to(self.device) for k, v in encode.items() if all(k != var for var in ['question', 'answer', 'task', 'prefix'])}
                # print(encode['input_ids'])
                # print(encode['attention_mask'])
                # print(encode['input_ids'].dim())
                # print(encode['attention_mask'].dim())
                ...
                output = tokenizer.batch_decode(tensor, skip_special_tokens=True)
                for text in output:
                    num += 1
                    prediction_df.loc[num] = [text]
    ...
    def text_to_encode(self,
                       ...
                       question_type: List or str = None,
                       prefix_type: List or str = None,
                       instruction_type: List or str = None,
                       task_type: List or str = None,
                       cache_path: str = None,
                       switch_to_model_ae: bool = False):
        ...
        questions = [None] * len(inputs) if question_type is None else question_type
        prefixs = [None] * len(inputs) if prefix_type is None else prefix_type
        tasks = [None] * len(inputs) if task_type is None else task_type        
        assert len(outputs) == len(inputs) == len(highlights) == len(questions) == len(prefixs) == len(tasks), \
        str([len(outputs), len(inputs), len(highlights), len(questions) == len(prefixs)])
        data = list(zip(inputs, outputs, highlights, questions, prefixs, tasks))

        # process in parallel/single
        config = {'tokenizer': self.tokenizer, 'max_length': self.max_length, 'prefix_type': prefix_type, 'prefix_index': 0,
        ...
        for i in tqdm(data):
            ...
            if len(out) > 40000 and cache_path is not None:
                ...
                out = []
        """
        elif type(instruction_type) != str:
            out = []
            files = []
            num = -1
            for i in tqdm(data):
                num += 1
                config['prefix_index'] = num
                f = EncodePlus(**config)
                e = f(i)
                if e is not None:  # remove overflow text
                    out.append(e)
    ...
    @staticmethod
    def get_data_loader(model, tokenizer, encode_list, batch_size: int = None, shuffle: bool = False, drop_last: bool = False):
        ...
        # print("\n\n\n\n\n")
        # print("Encode:", Dataset(encode_list))
        # print("\n\n\n\n\n")

        collate_fn_text = Collate_fn_text(tokenizer, 512)
        batch_size = len(encode_list) if batch_size is None else batch_size
        params = dict(batch_size=batch_size, shuffle=shuffle, drop_last=drop_last, \
                      num_workers=NUM_WORKERS, collate_fn=collate_fn_text)
...
class Collate_fn_text(object):
    def __init__(self, tokenizer, max_length):
        self.tokenizer = tokenizer
        self.max_length = max_length
        
    def __call__(self, data):
        #[{"input_id": tesor, "labels":tensor},{"input_id": tesor, "labels":tensor}{"input_id": tesor, "labels":tensor}{"input_id": tesor, "labels":tensor}]

        #[{"input_id": [tensor1, tensor2,...], "labels":[ tensor....]}]
        # print(data)

        for idx, encoded in enumerate(data):   
            # decode = self.tokenizer.decode(token_ids=encode, skip_special_tokens=True)
            # print("num:", encoded.keys())
            
            # attention_masks = encoded['attention_mask']
            # input_ids = encoded['input_ids']
            # labels = encoded['labels']
            questions = encoded['question']
            # answers = encoded['answer']
            prefixs = encoded['prefix']
            tasks = encoded['task']
            ...
            # decoded_input_ids = 
            # decode(token_ids=input_ids, skip_special_tokens=False)
            # decoded_input_ids = decoded_input_ids.replace("</s>", "").replace("<unk>", "").replace("<pad>", "")

            # decoded_labels = self.tokenizer.decode(token_ids=labels, skip_special_tokens=False)
            # decoded_labels = decoded_labels.replace("</s>", "").replace("<unk>", "").replace("<pad>", "")
            
            # print(decoded_labels)

            decoded_questions = self.tokenizer.decode(token_ids=questions, skip_special_tokens=False)
            decoded_questions = decoded_questions.replace("</s>", "").replace("<unk>", "").replace("<pad>", "")

            # decoded_answers = self.tokenizer.decode(token_ids=answers, skip_special_tokens=False)
            # decoded_answers = decoded_answers.replace("</s>", "").replace("<unk>", "").replace("<pad>", "")

            decoded_prefixs = self.tokenizer.decode(token_ids=prefixs, skip_special_tokens=False)
            decoded_prefixs = decoded_prefixs.replace("</s>", "").replace("<unk>", "").replace("<pad>", "")
            
            decoded_tasks = self.tokenizer.decode(token_ids=tasks, skip_special_tokens=False)
            decoded_tasks = decoded_tasks.replace("</s>", "").replace("<unk>", "").replace("<pad>", "")

                
            if decoded_tasks == "Random Masking":

                question_tokens = decoded_questions.split(" ")
                tokens_num = len(question_tokens)
                max_token_num = tokens_num - 1 if tokens_num != 1 else 1
                masked_token_nums = random.randint(1, max_token_num)
                masked_token_idxs = random.sample(range(tokens_num), masked_token_nums) 

                masked_question_tokens = []

                sentinal_token_num = 0
                for token_idx, question_token in enumerate(question_tokens):
                    if token_idx in masked_token_idxs:
                        if isinstance(question_token, list):
                            sentinal_tokens = []
                            for idx in range():
                                sentinal_token = f"<extra_id_{sentinal_token_num}>"
                                sentinal_tokens.append(sentinal_token)
                                sentinal_token_num += 1     
                        
                            masked_question_tokens.extend(sentinal_tokens)
                        else:
                            sentinal_token = f"<extra_id_{sentinal_token_num}>"
                            masked_question_tokens.append(sentinal_token)
                            sentinal_token_num += 1
                    else:
                        if isinstance(question_token, list):
                            masked_question_tokens.extend(question_token)
                        else:
                            masked_question_tokens.append(question_token)                                    

                masked_question = " " + " ".join(masked_question_tokens)
                masked_question_tokens = self.tokenizer.encode(masked_question, add_special_tokens=False)
                instruction_text = decoded_prefixs + ' "There are incomplete question: ' + self.tokenizer.decode(masked_question_tokens) + ' "'
                result = self.tokenizer(instruction_text, max_length=self.max_length,  padding="max_length", truncation=True)
                data[idx]['attention_mask'] = result['attention_mask']
                data[idx]['input_ids'] = result['input_ids']

            elif decoded_tasks == "Word Arrangement":
                # model_inputs = []
                ...
                tokens_num = len(question_tokens)
                # mask_ratio = random.randint(1, token_nums-1)
                max_token_num = tokens_num - 1 if tokens_num != 1 else 1
                masked_token_nums = random.randint(1, max_token_num)
                masked_token_idxs = random.sample(range(tokens_num), masked_token_nums) 
                ...
                # print(masked_question_tokens)
                masked_question = " " + " ".join(masked_question_tokens)
                masked_question_tokens = self.tokenizer.encode(masked_question, add_special_tokens=False)
                
                # self.max_length = 512
                # print(self.tokenizer.decode(masked_question_tokens))
                instruction_text = decoded_prefixs + ' "There are incorrect question: ' + self.tokenizer.decode(masked_question_tokens) + ' "'
                # instruction_text.split("")
                # text = self.tokenizer.decode(data[idx]['input_ids'])
                # text = self.tokenizer.decode(token_ids=text)
                # print("Decdoded Text: ", instruction_text)

                # instruction_text = self.tokenizer.decode(instruction_text)
                result = self.tokenizer(instruction_text, max_length=self.max_length,  padding="max_length", truncation=True)
                # print(result)
                # print(len(result))
                # self.max_length = 512
                # model_inputs.append(result['input_ids'])
                # model_attention_mask.append(result['attention_mask'])
        
                # if idx < 2:
                #     print("input token ids : ", text)
                #     print("input tokens : ",self.tokenizer.decode(result['input_ids']))
                # print(len(result['input_ids']), end="\n")
                data[idx]['attention_mask'] = result['attention_mask']
                data[idx]['input_ids'] = result['input_ids']
        ...
        for idx, feature in enumerate(data):
            ...
            if idx == 0:
                torch_labels = feature['labels'].unsqueeze(0)
                #  torch_questions = feature['question'].unsqueeze(0)
                # torch_answers = feature['answer'].unsqueeze(0)
                # torch_prefixs = feature['prefix'].unsqueeze(0)
                # torch_tasks = feature['task'].unsqueeze(0)

            elif idx != 0:
                # print("Label:", torch_labels)
                torch_labels = torch.cat([torch_labels, feature['labels'].unsqueeze(0)], dim=0)
                # torch_questions = torch.cat([torch_questions, feature['question'].unsqueeze(0)], dim=0)
                # torch_answers = torch.cat([torch_answers, feature['answer'].unsqueeze(0)], dim=0)
                # torch_prefixs = torch.cat([torch_prefixs, feature['prefix'].unsqueeze(0)], dim=0)
                # torch_tasks = torch.cat([torch_tasks, feature['task'].unsqueeze(0)], dim=0)
        
        """
        quenstion_length_list = []
        for idx, question in enumerate(labels_list):    
            quenstion_length_list.append(len(question))
        single_sentence_dict = dict.fromkeys(quenstion_length_list)
        single_sentence_list = list(single_sentence_dict)
        single_sentence_list.sort(reverse=True)
        print("\n\n")
        print(single_sentence_list)
        """
        # print(attention_mask_list)
        attention_masks = torch.tensor(attention_mask_list, dtype=torch.float32)
        input_ids = torch.tensor(input_ids_list, dtype=torch.long)

        # labels = [len(feature["labels"]) for feature in data]
        # print(max(labels))

        data = {'input_ids': input_ids, 'attention_mask': attention_masks, 'labels': torch_labels}

        # data = {'input_ids': input_ids, 'labels': torch_labels, 'attention_mask': attention_masks,
        # 'question': torch_questions, 'answer': torch_answers, 'prefix': torch_prefixs, 'task': torch_tasks}
