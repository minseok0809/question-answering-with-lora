...
def load_language_model(model_name,
    ...
    if pretrained_model_path == None:
        pretrained_model_path = "pretrained_model_folder"
    # https://blog.devgenius.io/fine-tuning-large-language-models-on-azure-machine-learning-358338f4e66a
    if (os.path.isfile(str(model_name) + '/pytorch_model.bin') == True and os.path.isfile(str(model_name) + '/adapter_model.bin') == False) or \
        (os.path.isfile(str(model_name) + '/pytorch_model.bin') == False and os.path.isfile(str(model_name) + '/adapter_model.bin') == False) and \
            (os.path.isfile(pretrained_model_path + '/pytorch_model.bin') == True and os.path.isfile(pretrained_model_path + '/traning start.txt') == True):
    ...
    elif os.path.isfile(str(model_name) + '/adapter_model.bin') == True and os.path.isfile(str(model_name) + '/pytorch_model.bin') == False:
        config = PeftConfig.from_pretrained(str(model_name), local_files_only=local_files_only, cache_dir=cache_dir, use_auth_token=use_auth_token) 
        
    elif os.path.isfile(pretrained_model_path + str(model_name) + '/pytorch_model.bin') == True and os.path.isfile(pretrained_model_path + str(model_name) + '/traning start.txt') == False and \
        type(model_name) != int:

        tokenizer = transformers.AutoTokenizer.from_pretrained(
            pretrained_model_path, cache_dir=cache_dir, local_files_only=local_files_only, use_auth_token=use_auth_token)
        
        config = transformers.AutoConfig.from_pretrained(
            pretrained_model_path, local_files_only=local_files_only, cache_dir=cache_dir, use_auth_token=use_auth_token) 

        f = open(pretrained_model_path + "/traning start.txt","w+")       
        for i in range(1):
            f.write("%d\r\n" % (i+1))
        f.close()

    elif os.path.isfile(pretrained_model_path + str(model_name) + '/pytorch_model.bin') == True and \
        type(model_name) == int:

        tokenizer = transformers.AutoTokenizer.from_pretrained(
           pretrained_model_path + str(model_name), cache_dir=cache_dir, local_files_only=local_files_only, use_auth_token=use_auth_token)
        
        config = transformers.AutoConfig.from_pretrained(
           pretrained_model_path + str(model_name), local_files_only=local_files_only, cache_dir=cache_dir, use_auth_token=use_auth_token) 

    elif  os.path.isfile(pretrained_model_path + str(model_name) + '/pytorch_model.bin') == False and type(model_name) != int:
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            model_name, cache_dir=cache_dir, local_files_only=local_files_only, use_auth_token=use_auth_token)
    ...
    elif type(model_name) == int:
        model = model_class(pretrained_model_path + str(model_name), **param)
    ...
    if model_adapter:
        ...
        if config.peft_type == 'LORA':
            if 't5' in model_name: 
                ...
                tokenizer = transformers.AutoTokenizer.from_pretrained(config.base_model_name_or_path, do_lower_case=False)
...
class TransformersQG:
    ...
    def __init__(self,
        ...
        self.tokenizer, self.model, config = load_language_model(
            self.model_name, self.pretrained_model_path, self.model_adapter, self.lora_rank, self.lora_alpha,
        ...
        try:
            ...
            if self.model_name_ae is None and type(model) != int:
