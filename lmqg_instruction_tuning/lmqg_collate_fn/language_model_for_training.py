...
def load_language_model(model_name,
    ...
    try:  
        ...
        param = {'config': config, "local_files_only": local_files_only,
                "use_auth_token": use_auth_token, "low_cpu_mem_usage": low_cpu_mem_usage, "cache_dir": cache_dir}
...
class TransformersQG:
    ...
    def encode_to_loss(self, encode: Dict):
        ...
        output = self.model(**{k: v.to(self.device) for k, v in encode.items() if all(k != var for var in ['question', 'answer', 'task', 'prefix'])})
